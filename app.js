...
bot.on('message', (msg) => {
    ...
    console.log("Received message:", botMsg);
...
Quer que te conte uma piada? hehe digite: 'conte uma piada'`;
    ...
    for (var i = 0; i <= especie.length; i++) {
        if (botMsg.indexOf(especie[i]) >= 0) {
            ...
            setTimeout(function() {
                bot.sendPhoto(msg.chat.id, "bettafish.jpg");
            }, 3000);
        }
    }
});
...
bot.onText(/\/berry/, (msg) => {
    var dateFromFile = fs.readFileSync('lastFoodDateTG.txt', 'utf8')
...
function saveDateToFile(dateToSave) {
    fs.writeFile("lastFoodDateTG.txt", dateToSave + "\n", function(err) {
        if (err) {
